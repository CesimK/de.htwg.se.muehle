...
case class Grid (var filled:Array[Char] = Array.empty, init:Boolean = false, num_fields:Int = 24) extends IGrid with GridCreateStrategy {
  val empty_field = "O"
  val empt_val = 'O'
  ...
  override def setGrid(grid: IGrid): Grid = this
}
